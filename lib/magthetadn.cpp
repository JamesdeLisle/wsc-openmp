...
MagThetaDn::MagThetaDn(std::string data_folder, Limits _lim) : \
...
double MagThetaDn::simpFac(int value, int max) {
...
std::vector<double> MagThetaDn::compute(int order) {
  std::vector<double> \
    ener = lim.space(0), \
    kPol = lim.space(1), \
    kAzi = lim.space(2);
