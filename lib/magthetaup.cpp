...
MagThetaUp::MagThetaUp(std::string data_folder, Limits _lim) : \
...
double MagThetaUp::simpFac(int value, int max) {
...
std::vector<double> MagThetaUp::compute(int order) {
  std::vector<double> \
    ener = lim.space(0), \
    kPol = lim.space(1), \
    kAzi = lim.space(2);
